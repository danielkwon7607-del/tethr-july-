...
## 3.5 Channel-ownership verification

*(Added 2026-07-10, CEO-approved. Closes the un-handbooked gap flagged in ADR 0011 §2a — onboarding proves no ownership of the channel address it is handed, so the channel is created **unverified**, and until it is verified tethr cannot contact the founder or recognize them on inbound.)*

A newly-onboarded channel is created **unverified**. tethr proves the founder owns the address before it will act on that channel, using the §18.5.2 authentication pattern:

- **Challenge.** On channel creation, tethr generates a one-time code and sends it to the address — the single outbound tethr will make to an unverified channel, and it carries nothing but the code. Only a hash of the code is stored; the code itself is never persisted.
- **Reply.** The founder sends the code back on that channel, as an ordinary message. tethr reads it, and **only a matching reply stamps `verified_at`.** A wrong or absent code leaves the channel unverified; repeated wrong codes lock the challenge.
- **Until verified.** tethr cannot initiate contact on an unverified channel (the first outbound requires a verified channel, §18.5.2), and inbound from an address with no verified channel is handled per §10.3 — an unverified-but-known channel is treated as an OTP reply, an entirely unrecognized address is replied-to-once and discarded.

Verification is owed by the entry boundary (the conversation surface that runs onboarding); the mechanism above lives in the messaging substrate and is wired into onboarding's channel-create step. The code send and the reply are both real tethr actions and follow the normal audit-before-dispatch and idempotency rules (§18.5.7).

## 3.6 The entry surface

*(Added 2026-07-12, Build 9a, CEO-approved — ADR 0015. `packages/onboarding` is a data-consuming library that takes a fully-populated input and seeds atomically; the **entry surface** owns the conversation that gathers it.)*

The onboarding conversation runs on a **web conversational surface** — a chat-style flow that asks the Path A/A2/B/C questions one at a time. It is deliberately not messaging-native: no founder channel exists before onboarding creates it, and §10.3 forbids storing pre-onboarding identity/PII against an unverified address, so the pre-channel conversation lives on the web entry boundary tied to the founder's session. Once onboarding completes and the channel verifies (§3.5), **messaging becomes the primary surface for the relationship** (§4.1); the web onboarding surface is the bootstrap, not where the work is driven.

- **Resume.** An in-progress onboarding is a draft keyed by an opaque token, so a founder who goes quiet mid-flow resumes at the exact next question without re-answering. A stalled draft **expires after 14 days**, clearing its partial PII.
- **Narrative seeds.** Several questions surface conviction, fear, and a stated commitment (origin story, feared outcome, one-year regret, stated builder-self). These are **not** trait estimates — they are the raw *stated* material §6.7 later reconciles against revealed behavior — and they persist in the onboarding episode with the same provenance discipline as every seed (§6.4), never discarded after the conversation.
- **Reach channel.** The founder's channel choice drives §3.5 verification. "Do not reach out" (and, until email is modeled as a channel, "email only") complete onboarding with **no channel and no OTP** — the model still seeds and Research still fires; tethr simply cannot initiate contact until a reachable, verified channel exists.
- **Path C keeps its origin signal.** When a Path C founder (no idea at all) surfaces a candidate and continues as Path A or B, the fact that they *arrived via C* is itself a process-sophistication signal (§3.2) and is not discarded in the re-route: their initial process-sophistication read starts below the native A/B founder's, then revealed behavior takes over (§6.7). All other seeded dimensions are unaffected.
...
Public Knowledge is a read-only corpus of canonical startup knowledge — 21,349 embedded chunks (verified live, 2026-07-07) drawn from sources such as Paul Graham, Steve Blank, SaaStr, First Round Review, and Hacker News. It lives in the `rag_corpus` table (schema canonical in `packages/db/migrations/0007`), embedded with OpenAI `text-embedding-3-small` (1536-dim); the query path must embed with exactly that model — enforced by dimension and model-pin guards in `packages/public-knowledge`. It is the received wisdom of how companies get built, made retrievable. It is **not** part of the Founder Model, holds nothing about any individual founder, and is never written to during use.
...
Inbound resolves by `(channel_type, address)` to exactly one channel, and its handling depends on what that channel is:

- **A verified founder channel** — the message is the founder's, threaded and fed to the model as normal.
- **A known-but-unverified channel** — the message is treated as an **OTP reply** (§3.5): tethr checks it against the live verification challenge and, on a match, verifies the channel. The message is a verification artifact, not a conversation turn — it is not stored as a thread message.
- **An unrecognized address** *(added 2026-07-10, CEO-approved)* — an address with **no matching channel** gets **one automatic reply** pointing at the onboarding link, and **the message is discarded.** Nothing is stored: no thread message, no phantom founder, no held "candidate identity." Onboarding is the entry point (Ch 3); storing speculative pre-onboarding identities would create privacy and identity-collision exposure the handbook does not sanction. The one reply is a real tethr-initiated action (audit-before-dispatch, idempotency — §18.5.7), sent at most once per address so a repeat or redelivered inbound never produces a second reply.
...
- **Supporting: Crunchbase** — funded competitors, funding momentum, market maturity. *(Build 7, CEO-approved: deferred on cost; the `funded_competition` signal is served by a second Serper source with funding-specific queries — a lower-fidelity stand-in, revisited when funded. ADR 0013.)*
...
*Built (Build 8, `packages/planning`, ADR 0014):* Planning consumes Research's `research.completed` (a verdict landing prompts Planning, §8.2) and generates a Plan as a validated dependency DAG — `sequence_index` by topological level, every dependency edge strictly ordered, cycles and dangling dependencies rejected (a Plan is never a flat list, §12.1). Estimated time is sized against the founder's `available_time` read (§6.9), conservatively under low confidence. All five §12.2 fields are NOT NULL (migration 0004); a malformed Action cannot persist. Founder pushback records a **deference** observation on the Founder Model write path (revealed side, §6.7) **and** re-sequences the Plan through the Build 2 internal intake, preserving completed Actions (the ratchet). Grounded in Public Knowledge (Ch 7, one of the two allowed consumers).
...
*Built (Build 8, `packages/validation`, ADR 0014):* Validation triggers on `plan.created` (§8.2), surfaces candidate assumptions grounded in Public Knowledge, and selects the **single** highest-risk one deterministically — `risk = impact × failure_likelihood` (expected lethality: how cheaply, and how likely, its failure kills the idea), argmax with an explicit tie-break so the second-highest can never be picked by float noise. The Experiment's five §13.2 fields are NOT NULL (migration 0004), and success/failure criteria are **frozen by a database trigger** (migration 0013) once the experiment leaves design or a result lands — a result can never be rationalized by editing the criteria after the fact. Sample size is personalized to founder capacity. Result ingestion routes pass → plan advancement, fail → re-planning, and **pivot → back into Research** (Ch 11), through the Build 2 intake, so the loop never dead-ends. The customer-facing send that produces a real result is Build 9; here the result is stubbed/founder-reported.
...
- **Messaging channels:** Photon Spectrum SDK (`@spectrum-ts/core` + providers) — iMessage/WhatsApp over a managed gRPC stream; SMS/RCS fallback is tethr's routing decision over Photon's service detection (Ch 10, Ch 21, ADR 0009). The stream is a long-lived connection: inbound runs as a persistent runner process, not a request function.
...
- Founder-facing access runs under a database role whose founder context (e.g. the authenticated JWT's founder claim, or an explicit `set_config` in workflow connections) is what the policy checks. **Service-role access that bypasses RLS is the narrow exception, not the norm:** confined to the background write path, migrations, and cross-founder operational jobs — each enumerated, each audited (§18.5.7), never used in request-scoped founder-facing code. *Enumerated exceptions (Build 5, ADR 0009):* the `resolve_channel_identity` security-definer function (inbound resolution is pre-identification — it returns only the owning founder, identity id, and verified flag for one address), and the delivery-reconciliation scan's cross-founder sweep of stale `message.send` ledger claims.
...
**The untrusted-input envelope (added 2026-07-09, Build 5, CEO-directed).** Inbound founder-channel content is untrusted input: no model tier consumes a raw inbound string. Before any model sees it, inbound content is wrapped in a delimited envelope that frames it as data — never instructions — with channel provenance, and embedded envelope markers are neutralized so content cannot break out of its frame. The rule is structural, not procedural: the envelope is a distinct type only the messaging substrate can produce, so a prompt path that wants founder words cannot accept a raw string. Workflow-event payloads carry identifiers, never message bodies — founder content stays in the encrypted store, not in queue payloads.
...
> Storage lives in Supabase/PostgreSQL with pgvector. This chapter fixes the conceptual shape; the **column-level schemas are implemented and canonical in `packages/db/migrations/`** (built 2026-07-06, Build 1) — one source of truth (Constitution VII), so this chapter describes and points rather than restates. Every schema is bound by Chapter 18.5: founder-scoped tables carry `founder_id` with forced, per-table-tested RLS (§18.5.4), and deletion follows the §6.16 layered timeline (`ON DELETE CASCADE` from `founders`, except the action ledger, which survives with its founder linkage severed). Embeddings are `vector(1536)`; a dimension change is a migration by design. The §18.5.7 ledger is the `action_ledger` table: claim + intent row are one atomic insert against a partial unique index, and a definite failure releases the claim while ambiguous outcomes hold it.
...
The `rag_corpus` table: 21,349 embedded chunks in pgvector (`vector(1536)`, `text-embedding-3-small`, HNSW cosine index), read-only for the app role, entirely separate from the Founder Model store (Ch 7), queried only by Planning and Validation via `packages/public-knowledge`. Deliberately the opposite of every founder-scoped table: shared, founder-free, no RLS, no write grants (§18.5.4 enumerated exception). Schema canonical in `packages/db/migrations/0007`.
...
- **Crunchbase** — funded-competitor and funding-momentum signal (supporting research signal). *(Deferred on cost as of Build 7; `serper_funding` is the interim stand-in — ADR 0013.)*
- **Resend** — outbound email delivery for outreach (Ch 14).
- **Photon Spectrum SDK** (`@spectrum-ts/core` + providers) — the messaging-channel layer: iMessage and WhatsApp over a managed gRPC stream, with service detection that tethr's own routing uses for SMS/RCS fallback (Ch 10, ADR 0009); the primary founder-facing surface and the integration with the hardest identity/continuity requirements (§19.4). Dedicated lines per founder (50 new conversations per line per day — the intervention policy's pacing is also a deliverability requirement); risks recorded in §10.2.
...
- **Build 1 schema decisions** (2026-07-06). Column-level schemas live in `packages/db/migrations/` as the single source of truth (Constitution VII). (1) RLS context is `current_founder_id()` reading transaction/session `app.founder_id`, with app access as the non-superuser `tethr_app` role — portable to Supabase by mapping the JWT founder claim to the same setting; rejected: policies hardwired to Supabase `auth.uid()` (unportable, untestable off-platform). (2) The §18.5.7 claim is a partial unique index on `(action_type, idempotency_key) where status <> 'failed'` — 'failed' releases by dropping out of the index, everything else holds; the failed row remains as history. (3) Founder deletion cascades through all founder-scoped tables except `action_ledger`, which sets `founder_id` null (§6.16 redaction: the record survives, the linkage doesn't). (4) Embeddings are `vector(1536)` (text-embedding-3-small class); re-dimensioning is a migration since embeddings must be recomputed anyway. (5) Episodes are append-only by trigger — only embedding backfill (from null) and tombstoning are updatable, so §19.1's "never updated in place" is a database guarantee, not a convention. (6) Post-review hardening (2026-07-06): `withFounderContext` drops privileges itself (`SET LOCAL ROLE tethr_app`) — isolation cannot depend on the DSN naming a low-privilege role, because superuser/owner connections bypass RLS unconditionally; the §18.5.7 claim index includes `founder_id`, making idempotency keys a per-founder namespace (no cross-founder blocking or probing); a malformed founder context is a clean deny (NULL), not an error on every query. **Accepted debt:** Postgres FK checks run unfiltered by RLS, so a valid foreign UUID confirms cross-founder existence — mitigated by UUID unguessability, noted in the migrations, revisit if UUIDs ever become enumerable.
- **Build 2 orchestration decisions** (2026-07-07). (1) Every irreversible external action inside a workflow goes through one wrapper (`runExternalAction`) composing the §18.5.7 ledger with durable steps; an **ambiguous prior outcome is never blindly retried** — it emits the internal `action.reconciliation-needed` event (degrade-to-asking, §8.5) and returns `needs-reconciliation`. Double-fire protection is two-layer: step memoization (Inngest replays skip completed steps) and the ledger claim (holds even if an event is redelivered as a fresh run). (2) The three §18.2 intakes are typed seams: inbound events **require** a dedup id (webhooks redeliver); scheduled scans are cron workflows whose sweep is injected by the capability that knows what to detect (orchestration detects, the Founder Model decides — §8.2); internal events flow through plain send. (3) Retry/backoff is Inngest's per-step machinery, not hand-rolled; tier-1/2 model calls route through the model router via a tier runner so the tier is named at the call site (§8.3, Ch 20). (4) Restart-survival is proven against the durable-engine **memoization contract** (a step store outliving the process); a live Inngest dev-server e2e is deferred to the first deployed environment, where the real event keys and serve() handler exist.

- **Build 3 Public Knowledge decisions** (2026-07-07, ADR 0006). (1) The corpus pre-exists in the live database as `rag_corpus` (21,349 chunks, `text-embedding-3-small`, 1536-dim, verified live); migration 0007 adopts that shape as canonical and supersedes 0003's never-populated `public_knowledge_chunks`. (2) Grounding retrieval lives in a dedicated `packages/public-knowledge` so the Ch 7 boundary is a package-dependency edge, enforced by a dependency-graph test (any package outside Planning/Validation depending on or importing it fails the suite); rejected: exporting from `packages/db` (substrate everyone already depends on — no seam). (3) Embeddings are a model-router capability with **no cross-provider fallback** — a fallback embedding model is a different vector space; one pinned model per corpus with hard dimension (1536) and model-pin guards; rejected: routing embeddings through the completion tier/fallback machinery. (4) The app path is SQL (`<=>` under HNSW) via the postgres client; the live `match_rag_corpus` RPC is an ingestion-side artifact used only by the verification script's REST fallback. Open: SQL-level live index verification awaits a live-database DSN; first deploy needs a migration-baseline step since the live schema pre-dates the chain.

- **Build 4 Founder Model decisions** (2026-07-07, ADR 0007). §6.15 implemented exactly as specced (shapes pinned by unit tests against the handbook's own worked examples). Implementation decisions the spec left open, now recorded: (1) `trait_observations` (migration 0008) is both the evidence ledger and the tuning instrumentation — confidence and estimates recompute from it, so every read is explainable and re-derivable; `policy_decisions` records every scoring outcome including veto applications. (2) v0 estimate mechanics: evidence-weighted mean per side; corroboration band 0.25 (tunable). (3) Corrections land on the revealed side at weight 1.0 — action policy follows revealed; stated is never silently overwritten (§6.7). (4) Graph supersession: same (source, relation) different target invalidates the old edge; identical re-assertion extends provenance. (5) Veto v0 constants: confidence gate 0.5, top band 0.7, intensity capped to gentle; a lone fresh observation cannot veto (no veto on a guess). (6) The write path (extract→abstract→reconcile) runs as durable steps on the Build 2 engine under RLS, deduped by event id; extractors are injected (Tier-1/2 model calls arrive with onboarding, Build 6). §6.16's inspection/correction data path ships here (`listTraits`, `traitHistory`, `applyCorrection`); the UI ships with the shell.

- **Build 5 gate — graph edge hardening** (2026-07-07, ADR 0008). An adversarial review gate over the Build 4 graph layer (traversal RLS and invalidation filtering: clean) surfaced three P1s, now fixed: (1) graph relations carry a caller-declared **cardinality** (`one`/`many`, migration 0009) — ADR 0007's blanket "different target supersedes" rule wrongly invalidated legitimately multi-valued facts; `one` now supersedes *all* live edges on its (source, relation), `many` coexists and dedupes identical facts. (2) The one-live-edge invariant is **database-enforced** via partial unique indexes — concurrent conflicting asserts fail loudly instead of leaving two live edges (same accepted posture as traits). (3) Graph context reads are **bounded by default** (fact limit 100). Also: entity matching normalizes case/whitespace so extraction noise cannot fabricate a false supersession, and the §6.5 write path now **proves episode ownership** (RLS-scoped visibility check) before writing — the event payload's founderId is untrusted input (§18.5.2). Tracked debt recorded in ADR 0008: attribute drift on re-assertion, unwindowed trait-ledger recompute, filtered-ANN under-fill at scale.

- **Build 5 messaging decisions** (2026-07-09, ADR 0009). (1) **Fallback correction:** Photon provides service *detection*, not automatic SMS/RCS fallback — fallback is tethr's routing decision over the §19.4 identities, with a verification-inheritance rule for sibling identities on an already-verified address; founder-visible behavior unchanged (§10.2, Ch 18, Ch 21 amended). (2) Two **enumerated §18.5.4 exceptions**: the `resolve_channel_identity` security-definer resolver (migration 0010 — inbound resolution is pre-identification and cannot bootstrap under forced RLS) and the delivery scan's cross-founder sweep of stale `message.send` ledger claims (aimed at the artifact that survives a dead workflow; reuses the reconciliation event id so incidents never double-ask). (3) The **§18.5.6 untrusted-input envelope** (CEO-directed) is structural: a branded type only the messaging substrate produces; events carry ids, never bodies. (4) **Ledger-only double-send guarantee**: the high-level Spectrum SDK exposes no transport dedup id; claim-before-dispatch is the guarantee, channel selection runs before the claim, the thread row is a durable step on the same key; `failed`/`delivered`/`read` are dormant status vocabulary pending real-line receipts. (5) **One dedicated line per founder**; the 50-conversations/line/day cap binds per founder (per-line counter = tracked debt). (6) **Initiation wires §6.15 scoring untouched** — §6.12's sentences map dimensions to candidates (accountability → style, cadence → dial, rhythm → timing via durable sleep, burnout → veto); "ask" means hold; Tier-2 compose and response-driven reweighting deferred to Build 6. (7) **Cadence adjustments are calibration writes**: explicit directive = correction (moves pacing), offhand mention = stated (deliberately weak alone). (8) **Unrecognized inbound drops the body** (address-only event) until Build 6's onboarding linkage. (9) Tests run against an in-memory channel twin; the Spectrum adapter is duck-typed and pinned by fakes; real-wire verification gated on a provisioned dedicated line. (10) The shell binds `TETHR_DEV_FOUNDER_ID` until Build 6 brings authenticated sessions; the §6.16 traits inspection surface ships with it.

- **Build 6 gate — live-index verification & prototype-schema quarantine** (2026-07-09, ADR 0010). The live DB password was reset and the DSN corrected to the IPv4 **session pooler** path (the direct host is IPv6-only, unreachable). With it: (a) `verify-build-3.ts` ran the full SQL path against live `rag_corpus` — index present (ivfflat), index used (EXPLAIN Index Scan), `retrieveGrounding()` relevant — all 10 checks green, closing the Build 3 live-index open item with no index to create. (b) Live inventory showed our migration chain was **never applied to live**: of 19 public base tables only `rag_corpus` is ours; the other 18 are prototype (incl. `founders`/`messages` colliding with our chain's names). All 18 were **renamed to a `legacy_` prefix** in one verified transaction — reversible by construction, `rag_corpus` untouched, run through `/careful`. Tracked debt (ADR 0010): constraint/index names are unprefixed, so the first-deploy migration-baseline step must still resolve `founders_pkey`-class collisions.

- **Build 6 onboarding decisions** (2026-07-09, ADR 0011). (1) A new owning subsystem `packages/onboarding` composes existing seams (invents no product behavior, no policy — Constitution XII). (2) **Founder creation is a service-role write**, everything after is founder-scoped: the `founders` table has no insert policy by design (creation precedes the scope), the same bootstrap shape as inbound resolution (ADR 0009). (3) **The cold-start seed set is the UNION of §3.3's dimensions and the four the §6.12 initiation policy reads** — so the very first proactive contact reads real seeds and is conservative because a single **stated** observation is low-confidence by the §6.15 math (no new policy logic; the harness link is verified end-to-end: recorded `confidence_gate` >0 but <0.3, `nudge.hard` never acts). (4) All seeds are `stated`, low-confidence; the entry path itself signals process sophistication (none < problem < idea, §3.2); self-reports optional (§3.1). (5) **Research is auto-triggered, stubbed** — onboarding completion emits `onboarding.completed` through the internal-event intake; the stub advances the company to `researching` (§3.4) and Build 7 fills the pipeline; the founder never asks. (6) The four items the Build 5 handoff deferred into Build 6: write-path extractors (`createModelExtractors`, Tier-1/2 + Zod boundary validation), Tier-2 compose (`createInitiationCompose`), and response-driven reweighting (`registerResponseLearning`, a decoupled workflow on the inbound event, §6.9/§10.4) all **shipped and tested against fakes**; **shell auth shipped its data-layer half** (`founders.auth_user_id` linkage + `founderIdForAuthUser` resolver), the Next.js session plumbing (`@supabase/ssr`) deferred. **The unrecognized-inbound consumer is deferred as a Confusion-Protocol stop, not a budget cut** — it presupposes a text-before-onboarding flow Chapter 3 does not specify; building it would invent un-handbooked behavior (Constitution I) and needs a handbook amendment first. (7) **Pre-tag review hardening:** onboarding creates the founder's channel **unverified** (stamping `verified_at` without proof is a channel-takeover vector; the verification mechanism — OTP / proven inbound — is an un-handbooked gap owed by the entry boundary), founder creation + seeding run in **one atomic transaction** with **idempotent auth-linked resume** (no orphaned half-seeded founder, no unique-collision on retry), and response-driven reweighting **requires a delivered (`sent`) outbound** in the window (an act that never reached the founder is not efficacy). Live model wiring in the runner is production-placement debt (joins deploy-staging, ADR 0009).

- **Gate 0 — channel-ownership verification & unrecognized-inbound handling** (2026-07-10, CEO-approved, ADR 0012). Closes the two ADR 0011 gaps as handbook amendments (§3.5, §10.3) plus implementation. (1) **OTP channel verification.** A newly-onboarded channel is created unverified; tethr sends a one-time code (the one outbound sanctioned to an unverified channel), the founder replies with it, and only a matching reply stamps `verified_at`. The code is HMAC-peppered (§18.5.5) and only its hash is stored; verification is a security-definer mutation (`verify_channel_otp`, migration 0011) — the same enumerated §18.5.4 pre-identification exception class as `resolve_channel_identity`, here mutating — with a 5-attempt cap and 10-minute expiry. The challenge row is created inside Build 6's atomic onboarding tx; the code is sent post-commit (audit-before-dispatch, idempotent on the identity, §18.5.7). (2) **Unrecognized-inbound reply-and-discard.** An inbound from an address with no matching `channel_identity` gets one onboarding-link reply and is discarded — no `messages` row, no phantom founder. The reply is founderless (pre-identification), so it uses a system-scoped null-founder `action_ledger` row through the `claim_system_action`/`record_system_action_outcome` definers (migration 0011) — reusing the one audit substrate rather than a parallel store — idempotent per address so a repeat sender never earns a second reply. The raw address stays in-process for the reply and is never put in the event (preserving ADR 0009's §18.5.6 minimization). `/cso` reviewed the surface: no blocking findings; tracked pre-production hardening (in ADR 0012) — boot-time validation of the OTP pepper (§18.5.5), a global cap on unrecognized replies, and `FOR UPDATE` on the challenge select. The Build 5 conflation (unverified inbound also emitting the unrecognized event) is corrected: "unrecognized" now means *no matching channel* only.
- **Build 7 — Research pipeline** (2026-07-10, ADR 0013). `packages/research` builds Ch 11: the four-stage pipeline (idea stress-test → competitor landscape → market-signal synthesis → verdict) as durable steps, auto-triggered from `onboarding.completed` (§3.4) — research owns that consumer seam (Constitution XII), onboarding only emits. **Weighted synthesis, not averaged** (§11.2): demand and competition are computed from *disjoint* signal sets with explicit weights, combined by a non-linear verdict rule (`strong_signal`/`weak_signal`/`pivot`), evidence-linked back to source URLs (§11.4); the Tier-2 model writes the narrative, the deterministic rule writes the decision. **Rec #5 closed** — a per-founder cost budget (spend ledger `research_spend`, migration 0012) with check-before-charge back-pressure: exhaustion emits `research.paused` and writes no verdict (graceful stop + founder-ask, §8.5); the burnout veto (§6.14) feeds the same pause path. **Rec #6 closed** — per-source 429 fail-fast (no hammering), a staleness-typed cache (`research_cache`, per-source TTL), and ToS notes (ADR 0013). **Source scope (CEO):** xAI X Search (primary), Hacker News (free), Serper — plus a second Serper source (`serper_funding`) with funding-query patterns as the funded-competition stand-in; **Crunchbase deferred on cost** (ADR 0013, lower-fidelity substitute, revisit when funded). Research does **not** touch Public Knowledge (Ch 7 boundary, dependency-test still green). Tier-2 synthesis / Tier-1 classification via the model router; every source call is behind the typed port, never a raw SDK. Deferred (deploy-time, joins deploy-staging): live `ModelRouter` + http-source wiring in the runner, provider keys into the boot config schema. Tag `build-7-research`. Build 8 (`build-8-planning-validation`, 2026-07-11, ADR 0014) — `packages/planning` + `packages/validation`, the two consumers of the verdict: Planning generates a sequenced dependency-DAG Plan on `research.completed` (five NOT NULL Action fields, estimated time sized against `available_time`, pushback → deference write + ratcheted re-sequence), Validation designs an Experiment against the single highest-risk assumption (`impact × failure_likelihood` argmax, criteria frozen by migration 0013's DB trigger) and routes results (pass → advance, fail → re-plan, pivot → back into Research so the loop never dead-ends); both grounded in Public Knowledge (Ch 7 boundary green), 216 tests green. Deferred: true cross-model /codex check (codex CLI absent — ran as a Claude subagent), machine-evaluable criteria falsifiability (Build 9 handbook amendment), `plan.advance`/`plan.replan` consumers (Build 9).
- **Gate 1 — the onboarding entry-surface UI is deferred to Build 9** (2026-07-10). The conversation surface that asks the founder the Path idea/problem/none questions and calls `runOnboarding` is deferred to Build 9's MVP-completion work, with the deferred outreach send. The library-vs-surface split from Build 6 (ADR 0011) is **intentional**: `packages/onboarding` is a data-consuming library that composes existing seams (Constitution XII); the entry boundary owns the conversation, question-gathering, and — now — the OTP challenge/reply UX (§3.5). Recorded so a later session does not rediscover it as an open item.

- **Build 8 — Planning & Validation** (2026-07-11, ADR 0014). `packages/planning` and `packages/validation` build Ch 12–13 as the two consumers of the Research verdict — sibling owning subsystems (Constitution XII), both Tier-2 generation grounded in Public Knowledge (Ch 7's two allowed consumers, boundary test green) and personalized by the Founder Model. **Planning:** `research.completed` → a Plan generated as a validated dependency **DAG** (topological `sequence_index`, strict dependency ordering, cycle/dangling rejection — never a flat list, §12.1); estimated time sized against `available_time` (§6.9, conservative under low confidence); the five §12.2 Action fields NOT NULL (malformed rejected). Founder pushback writes a **deference** observation (revealed side, §6.7) **and** re-sequences through the Build 2 intake, preserving completed work (the ratchet, §12.4). **Validation:** `plan.created` → design an Experiment against the **single** highest-risk assumption, selected deterministically as `risk = impact × failure_likelihood` (expected lethality — the model surfaces scored candidates, a pure argmax with an explicit tie-break decides, mirroring ADR 0013's synthesis split); the five §13.2 fields NOT NULL and success/failure **criteria frozen by a DB trigger** (migration 0013) once the experiment leaves design or a result lands (structural anti-rationalization, §13.2). Result ingestion routes pass → plan advancement, fail → re-planning, **pivot → Research** (Ch 11) through the internal intake so the loop never dead-ends (Research owns the `validation.pivot` consumer; Validation only emits). Distinct from a research `pivot` verdict, which re-enters at Planning (§11.4), not Research. Customer send stubbed (Build 9). **Tracked debt:** /codex ran as a fresh-context Claude subagent, not a true different model (codex CLI absent) — a real cross-model pass on the sequencing/risk-selection logic is deferred; falsifiability enforcement is a cheap `success≠failure` guard only (machine-evaluable criteria regions would be new product behavior, flagged for a Build 9 handbook amendment); `plan.advance`/`plan.replan` consumers are Build 9. Tag `build-8-planning-validation`.

Open decisions carried forward: empirical tuning of the §6.15 calibration constants; column-level schemas beyond messaging identity (Ch 19); the extensible dimension set as new policies are defined (§6.3); ~~per-founder cost budgets and back-pressure~~ **resolved Build 7 (Rec #5, ADR 0013)**; ~~research-source quotas, caching, and ToS policy~~ **resolved Build 7 (Rec #6, ADR 0013)**; the §6.15 instrumentation-and-tuning plan (with the Founder Model build); the irreversible-action approval UX (before the outreach build); the Grok voice API spec (before the voice fast-follow); a real Crunchbase integration (deferred on cost — `serper_funding` stand-in, ADR 0013).
...
No product capability is built yet; the sequence above is the plan. The engineering substrate exists (ENGINEERING_OS §7): Build 0 (2026-07-06) — TypeScript monorepo, CI, fail-fast three-environment config, the idempotency+audit substrate, and the workflow (Inngest) and model-router (in-house on the Vercel AI SDK) abstractions with one adapter each — ADRs in `/docs/decisions`. Build 1 (`build-1-data`, 2026-07-06) — the full Ch 19 schema as reversible migrations with forced, per-table-tested RLS (§18.5.4) and the §18.5.7 action ledger, integration-tested against real Postgres+pgvector in CI. Build 2 (`build-2-orchestration`, 2026-07-07) — the three trigger intakes, tier routing through the model router, the idempotent external-action wrapper with audit-before-dispatch and degrade-to-asking, and restart-survival proven against the durable memoization contract (Ch 23). Build 3 (`build-3-knowledge`, 2026-07-07) — the Public Knowledge grounding path over the live `rag_corpus` (21,349 chunks): `packages/public-knowledge` retrieval with pinned-model and dimension guards, the test-enforced Planning/Validation-only boundary, and live relevance verified via `scripts/verify-build-3.ts` (Ch 23, ADR 0006). Build 4 (`build-4-founder-model`, 2026-07-07) — the flagship: `packages/founder-model` with the §6.15 calibration v0 (tested against the handbook's own numbers), the four-layer stores (evidence-ledger traits, bi-temporal graph, hybrid §6.8 retrieval), the §6.5 background write path on the orchestration engine, the hard burnout veto measurably capping intervention, and the §6.16 inspection/correction data surfaces (Ch 23, ADR 0007). Build 5 (`build-5-messaging`, 2026-07-09) — the founder-facing surface: `packages/messaging` (inbound resolution via the 0010 security-definer resolver, ordered/deduped one-thread-per-founder persistence, the §18.5.6 untrusted-input envelope, §18.5.7-ledgered sends with tethr-owned SMS/RCS fallback, the delivery-reconciliation scan, and §6.12-driven initiation with the burnout veto measurably suppressing under load — all acceptance-tested against an in-memory channel twin), the persistent stream-runner entry point, and the thin Ch 4 shell (Company State + §4.5 cadence surface, Plan, Experiment, §6.16 traits inspection) rendering from RLS-scoped reads (Ch 23, ADR 0009). Build 6 (`build-6-onboarding`, 2026-07-09) — Chapter 3: `packages/onboarding` with the three entry paths, §6.13 cold-start seeding (stated, low-confidence reads on the union of §3.3's dimensions and the four the §6.12 policy reads, so the first proactive contact is personalized and provably conservative under low confidence), and the §3.4 auto-trigger into Research (stubbed entry point — the trigger wired through the internal-event intake, not the pipeline). Folded in from the Build 5 handoff: the model-backed write-path extractors, Tier-2 initiation compose, and response-driven policy reweighting (all tested against fake model runners), plus shell auth's data-layer half (`auth_user_id` linkage + resolver). Deferred with reasons: the unrecognized-inbound consumer (needs a text-before-onboarding flow Ch 3 doesn't specify — Constitution I), live model wiring in the runner (deploy-staging debt), and the Next.js session plumbing (`@supabase/ssr`) (Ch 23, ADR 0011). Gate 0 (2026-07-10, ADR 0012) — channel-ownership verification via OTP (a security-definer verify + peppered code, the challenge in onboarding's atomic tx, the code sent post-commit) and unrecognized-inbound reply-and-discard (one founderless onboarding-link reply through a system-scoped ledger, no stored message), closing the two ADR 0011 gaps as amendments §3.5/§10.3. Build 7 (`build-7-research`, 2026-07-10, ADR 0013) — `packages/research`: the four-stage pipeline auto-triggered from onboarding, weighted (not averaged) synthesis into an evidence-linked `strong/weak/pivot` verdict, per-founder cost budget with stop-and-ask back-pressure (Rec #5) and the burnout tie-in, per-source 429 fail-fast + staleness-typed cache (Rec #6), sources xAI/HN/Serper with a Serper funding-query stand-in for the cost-deferred Crunchbase; Public Knowledge untouched (Ch 7 boundary green); acceptance-tested against fakes, live wiring deploy-time. Build 9a (`build-9a-entry-surface`, 2026-07-12, ADR 0015) — `packages/entry`: the **web conversational entry surface** (§3.6) owning the Path A/A2/B/C question flow (verbatim copy, explicit state machine, the A→A2 and Path-C candidate→re-entry branches), token-keyed **resume** with a 14-day expiry sweep (migration 0014 `onboarding_sessions`), answer→`OnboardingInput` mapping, and **idempotent completion** into `runOnboarding` via `founders.onboarding_session_id`, now carrying `narrativeSeeds`/`buildingContext` into the onboarding episode with provenance (§6.4). The OTP challenge fires on completion and a re-challenge/**resend** path (ADR 0012 §9) is built; "do not reach out"/"email only" seed with no channel/OTP. Gap A (`customer_contact_avoidance`, `working_rhythm` active-hour) deferred to behavioral learning (§6.7); email-as-channel is unmodeled (`ChannelType` has no email) and flagged. 30 entry tests + full suite (246) green on a local pgvector cluster. Deferred (this build's remaining scope / deploy-time): the founder-facing web UI wiring (server actions + client surface + status view), live messaging/engine/OTP-secret wiring, and Path C's Tier-2 candidate-model wiring. The design-level open items flagged in v0.3 are **resolved at design level**:
...
- ~~Column-level schemas for the rest of §19 beyond messaging identity~~ — **resolved 2026-07-06 (Build 1)**: implemented in `packages/db/migrations/` with per-table RLS tests (Ch 19, Ch 23).
