...
**gstack** (Garry Tan's suite) — the **primary engineering spine**. A large set of opinionated, role-based slash commands spanning the lifecycle: `/office-hours` (forces assumptions into the open before code — the front-end of planning), a **Confusion Protocol** that stops Claude guessing on architectural decisions, `/review` (catches unnecessary complexity and drive-by edits), `/oracle` (product-conscience AST scan of the codebase), `/ship` (turns tasks into verifiable, test-first goals), `/qa` (staging/e2e verification), `/retro` (post-work learnings), `/document-release`, and safety guards `/careful` (warns before destructive commands: `rm -rf`, `DROP TABLE`, force-push, `git reset --hard`), `/freeze` (locks edits to one directory during debugging), `/guard` (both), `/investigate` (auto-freezes to the module under investigation). *Limit / overlap:* gstack also ships a memory-sync feature that overlaps claude-mem — **we disable gstack memory-sync and use claude-mem for memory** (§4, Memory). gstack's planning/review overlap Superpowers; the routing below divides them explicitly.
...
|---|---|---|---|---|
| **Planning** | gstack `/office-hours` | Superpowers `writing-plans` | gstack owns product/assumption framing and the Confusion Protocol; Superpowers owns the micro-task plan with real code + test gates | Approved design intent + a task-gated plan, no `TBD` |
...
| **Review** | gstack `/review` + `/oracle` | Superpowers reviewer subagent, Ponytail `/ponytail-review` | Three lenses, not duplication: `/review` for complexity/drive-by edits, `/oracle` for product conscience, reviewer subagent for spec+quality on large diffs, Ponytail for a delete-list | Review verdict + delete-list + spec-compliance sign-off |
| **Testing** | Superpowers `test-driven-development` | gstack `/qa` | Iron-law red/green is the test engine; `/qa` covers staging/e2e | Failing-test-first, then a green suite |
...
| **Memory** | claude-mem | Handbook (stable truths) | Silent cross-session recall; **gstack memory-sync is disabled to avoid duplication** | Injected prior context at session start; stable conclusions promoted to handbook |

**Overlap rulings (to satisfy "never duplicate a skill"):**
- *Planning:* gstack owns assumptions/framing; Superpowers owns the detailed plan. Do not run two brainstorms.
- *Memory:* claude-mem only; gstack memory-sync off.
- *TDD:* Superpowers is the TDD engine; gstack `/ship` orchestrates the goal but delegates red/green to Superpowers.
- *Review:* layered lenses (complexity / product-conscience / spec-quality / delete-list), each distinct — not four passes of the same thing.
